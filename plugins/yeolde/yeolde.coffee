do ->
    ...
    word = (original, replacement) ->
        unless Array.isArray(original)
            original = [original]
        unless Array.isArray(replacement)
            replacement = [replacement]

        for wrd in original
            words.push([new RegExp("\\b#{wrd}\\b", "gi"), replacement])
    ...
    yeolde = (message) ->
        ...
        message = message.replace insults, ->
        ...
        if ["!", "?", "", ",", "."].indexOf(message[message.length - 1]) is -1
            message += "."

        return "#{startmsg} #{message} #{endmsg}"

    confetti.encool.register('yeolde', yeolde)
